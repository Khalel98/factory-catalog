...
export default defineEventHandler(async (event) => {
  try {
    ...
    const { productId, categoryId, specifications, language = 'ru' } = body;

    if (!productId || !categoryId || specifications === undefined) {
      throw createError({
        statusCode: 400,
        statusMessage: "Необходимы productId, categoryId и specifications",
      });
    }
    ...
    let specificationsIndex = -1;
    let columnName = '';
    
    if (language === 'kk') {
      specificationsIndex = headers.findIndex(
        (h) =>
          h?.toLowerCase() === "specificationskk" ||
          h?.toLowerCase() === "specifications_kk" ||
          h?.toLowerCase() === "specifications_kk"
      );
      columnName = 'specificationsKK';
    } else {
      // По умолчанию русский
      specificationsIndex = headers.findIndex(
        (h) =>
          h?.toLowerCase() === "specificationsru" ||
          h?.toLowerCase() === "specifications_ru" ||
          h?.toLowerCase() === "specifications_ru" ||
          h?.toLowerCase() === "specifications" ||
          h?.toLowerCase() === "specification"
      );
      columnName = 'SpecificationsRU';
    }
    ...
    // Обновляем specifications в Google Sheets
    // Вычисляем букву колонки (A=1, B=2, ..., Z=26, AA=27, и т.д.)
    let columnLetter = '';
    let colNum = specificationsIndex + 1;
    while (colNum > 0) {
      colNum--;
    ...
    const specificationsRange = `${categorySheet.properties.title}!${columnLetter}${productRowIndex}`;
    await sheets.spreadsheets.values.update({
      spreadsheetId,
      range: specificationsRange,
    ...
    for (const category of categories) {
      ...
      // Локализованные поля Specifications
      const specificationsRUIdx = headers.findIndex(
        (h) =>
          h?.toLowerCase() === "specificationsru" ||
          h?.toLowerCase() === "specifications_ru" ||
          h?.toLowerCase() === "specifications_ru" ||
      ...
      const specificationsKKIdx = headers.findIndex(
        (h) =>
          h?.toLowerCase() === "specificationskk" ||
          h?.toLowerCase() === "specifications_kk" ||
          h?.toLowerCase() === "specifications_kk"
      ...
      for (let i = 1; i < productRows.length; i++) {
        ...
        product.priceComplectationRU = priceComplectationRU;
        product.priceComplectationKK = priceComplectationKK;
        product.priceComplectationInfo = priceComplectationRU || priceComplectationKK || "";

        // Specifications (текст/HTML) - используем RU или KK в зависимости от наличия
        const specificationsRU = row[specificationsRUIdx]?.trim() || "";
        const specificationsKK = row[specificationsKKIdx]?.trim() || "";
    ...
    return {
      success: true,
      message: "Технические характеристики успешно обновлены и каталог регенерирован",
    };
  } catch (error) {
    console.error("Error updating product specifications:", error);
    throw createError({
      statusCode: 500,
      statusMessage: error.message || "Ошибка при обновлении технических характеристик",
    });
  }
});
