<template>
  <div class="admin-page spacing">
    <div class="card">
      <h1>Управление подбором веществ</h1>
      <p class="muted">
        Редактируйте вещества и привязывайте товары по ID. Изменения сохраняются в Google Sheets и локально.
      </p>
      ...
      <template v-else>
        ...
        <template v-else>
            ...
            >
              <div class="substance-row">
                <div class="substance-field">
                  <label>ID</label>
                  <input v-model="sub.id" type="text" placeholder="propan" />
                </div>
                <div class="substance-field flex-grow">
                  <label>Название</label>
                  <input v-model="sub.name" type="text" placeholder="Пропан (C3H8), % об. доли" />
                </div>
                <div class="substance-field flex-grow">
                  <label>Название (каз.)</label>
                  <input v-model="sub.nameKz" type="text" placeholder="Пропан (C3H8), % көлемдік үлес" />
                </div>
                <button
                  v-if="substances.length > 1"
                  type="button"
                  class="btn danger small"
                  @click="removeSubstance(idx)"
                >
                  Удалить
                </button>
              </div>
              <div class="substance-field">
                ...
                <input v-model="sub.description" type="text" placeholder="" />
              </div>
              <div class="substance-field">
                <label>Описание (каз., необязательно)</label>
                <input v-model="sub.descriptionKz" type="text" placeholder="" />
              </div>
              <div class="substance-field">
          ...
          <div class="actions">
            <button type="button" class="btn secondary" @click="addSubstance">
              + Добавить вещество
            </button>
            <button
              type="button"
            ...
            >
              {{ saving ? 'Сохранение...' : 'Сохранить вещества' }}
            </button>
          </div>
      ...
      <div class="info-section" style="margin-top: 32px">
        ...
        <ul class="info-list">
          <li>Лист «Substances» в Google Sheets: колонки id, name, nameKz, description, descriptionKz, productIds</li>
...
function onProductIdsInput(idx, e) {
  ...
  substances.value[idx].productIds = ids;
}

function syncProductIdsStr() {
  productIdsStr.value = substances.value.map((s) =>
    Array.isArray(s.productIds) ? s.productIds.join(', ') : ''
  );
}

function addSubstance() {
  substances.value.push({
    id: '',
    name: '',
    nameKz: '',
    description: '',
...
function removeSubstance(idx) {
  substances.value.splice(idx, 1);
...
async function loadSubstances() {
  ...
  try {
    ...
    substances.value = Array.isArray(data)
      ? data.map((s) => ({
          id: s.id,
          name: s.name,
          nameKz: s.nameKz || '',
...
async function saveSubstances() {
  ...
  try {
    const res = await $fetch('/api/substances-update', {
      method: 'POST',
      body: { substances: substances.value },
    });
